...
- In **Elastic Beanstalk &rarr; Create Application &rarr; Add users** 
 ...
 - **Manual Method** to Build and Deploy the Artifact from local system
   ...
   - In **Elastic Beanstalk &rarr; Environments &rarr; app-env** click the **Configuration**. In the **Load balancer** section **&rarr; Edit**. Select the **Processes** and the Health check Path from **'/' to '/login'**. Also, checkmark the **Stickiness policy enabled** and **Save**.
